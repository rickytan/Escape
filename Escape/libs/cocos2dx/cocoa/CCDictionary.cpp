...
{
    return objectForKey(std::string(key));
}
...
{
    return valueForKey(std::string(key));
}
...
void CCDictionary::setObject(cocos2d::CCObject *pObject, const char *key)
{
    setObject(pObject, std::string(key));
}
